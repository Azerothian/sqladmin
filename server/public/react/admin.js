...
RawData = (function(_super) {
  ...
  function RawData() {
    ...
    this.on("change:query", (function(_this) {
      return function() {
        var options;
        if (_this.get("db") == null) {
          debug("unable to send, no db set");
          return;
        }
        _this.get("dataset").reset();
        options = {
          url: "/api/raw",
          dataType: "json",
          type: "POST",
...
RawInterface = React.createClass({
  runQuery: function() {
    return AppData.get("rawdata").set("query", this.refs.editor.getValue());
  },
  render: function() {
    return div({}, Row({}, Col({
      xs: 12,
